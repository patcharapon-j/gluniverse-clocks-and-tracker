...
- **Delving Mode** — a turn-driven mode of play for dungeon crawls, hauntings, descents — anywhere the *passage* of time matters but the wall-clock reading doesn't. Toggle it on from the HUD dock at any time; `game.time` keeps advancing under the hood (so effects still expire and the weather still walks), but the clock readout steps aside for a **turn counter** and the featured resource's atmosphere.
  ...
  - **Delving resources** — staged dice pools (torches, corruption, signal, air…). Each turn rolls every resource's current-stage pool (drop dice ≤ the discard range, exactly like the Tracker's Resource Pool) and posts one **consolidated Turn card**; the featured resource's roll animates as **real, physically-simulated 3D dice rolled *inside the chat card*** (three.js for rendering + cannon-es for rigid-body physics — the Dice So Nice technology, but contained in the card rather than a full-screen overlay): the dice are thrown into a small walled arena, bounce off the floor, the walls and each other, and tumble to a natural rest under gravity — so every roll looks different. The simulation isn't rigged; once the dice settle, the *true* rolled value is painted onto whichever face landed up, then the canvas fades to the baked static result for scrollback. *(three.js + cannon-es load on demand; if either can't load — offline / CSP — it falls back to a contained 2D tumble, then the static result.)* When a pool empties, the resource **shifts to its next, worse stage** and refills; the final stage clamps and persists.
