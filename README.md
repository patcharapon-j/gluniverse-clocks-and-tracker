...
- **Shift-level (watch) mode** — when stretch-by-stretch detail is more than your table needs, the GM flips a world-wide toggle on the HUD dock: the HH:MM clock and stretch meter step aside and a 4-quadrant **watch dial** promotes the current shift to hero. Time still advances and stores in stretches under the hood (so effect expiry and other modules are unaffected); only the display changes, and players see the simpler view too. Set the default in module settings.
