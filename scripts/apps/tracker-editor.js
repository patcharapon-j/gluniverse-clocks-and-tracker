/**
 * TrackerEditor — GM create/edit dialogs for trackers.
 *
 ...
 * straight to the form for the existing tracker's type. All writes go through
 * TrackerStore, which broadcasts to players via the world setting.
 */
...
export class TrackerEditor {
  ...
  static async create() {
    ...
    if (!type) return;
    const data = await this._promptConfig(type, TrackerStore.makeNew(type));
    if (!data) return;
    await TrackerStore.create(type, data);
  }

  /** Edit an existing tracker by id. */
  static async edit(id) {
    if (!game.user.isGM) return;
    const t = TrackerStore.get(id);
    if (!t) return;
    const data = await this._promptConfig(t.type, t);
    if (!data) return;
    await TrackerStore.update(id, data);
  }
  ...
  static _fields(type, t) {
    ...
    switch (type) {
      ...
      case "pool":
        ...
        parts.push(row(L("GLCT.tracker.field.playerRoll"), check("playerRoll", t.playerRoll)));
        break;
      case "task":
      case "hazard":
  ...
  static _read(type, form) {
    ...
    switch (type) {
      ...
      case "pool": return {
        ...
        current: nn("current", nn("count", 5)), playerRoll: ck("playerRoll")
      };
      case "task":
      case "hazard": return {
  ...
  static async _promptConfig(type, t) {
    try {
      return await DialogV2.prompt({
        window: { title: `${L("GLCT.tracker.edit")} · ${L(`GLCT.tracker.types.${type}`)}`, icon: "fa-solid fa-pen" },
        content: `<div class="glct-trk-form">${this._fields(type, t)}</div>`,
        ok: {
          label: L("GLCT.editor.save"),
          callback: (event, button) => this._read(type, button.form)
        },
        rejectClose: false
      });
    } catch { return null; }
